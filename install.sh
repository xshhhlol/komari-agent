...
# Function to drop a self-contained uninstall script next to the agent binary
create_uninstall_script() {
    local uninstall_path="${target_dir}/uninstall.sh"
    log_step "Creating uninstall script: ${GREEN}$uninstall_path${NC}"

    # Header: bake the install-time service name / dir / OS into the script.
    cat > "$uninstall_path" << EOF
#!/bin/bash
# Komari Agent uninstall script (auto-generated by install.sh).
# Stops and removes the agent service, then deletes the install directory.
service_name="${service_name}"
target_dir="${target_dir}"
os_name="${os_name}"
EOF

    # Body: single-quoted heredoc so these vars expand at uninstall time, not now.
    cat >> "$uninstall_path" << 'EOF'

GREEN='\033[0;32m'; YELLOW='\033[0;33m'; RED='\033[0;31m'; NC='\033[0m'

if [ "$(id -u)" -ne 0 ] && [ "$os_name" != "darwin" ]; then
    echo -e "${RED}Please run as root: sudo ${target_dir}/uninstall.sh${NC}"
    exit 1
fi

echo -e "${YELLOW}Uninstalling Komari Agent (service: ${service_name})...${NC}"
...
if command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files 2>/dev/null | grep -q "${service_name}.service"; then
    ...
    user_plist="$HOME/Library/LaunchAgents/com.komari.${service_name}.plist"
    if [ -f "$system_plist" ]; then
        echo "Removing system launchd service..."
        launchctl bootout system "$system_plist" 2>/dev/null || true
        rm -f "$system_plist"
    fi
    if [ -f "$user_plist" ]; then
        echo "Removing user launchd service..."
        launchctl bootout "gui/$(id -u)" "$user_plist" 2>/dev/null || true
        rm -f "$user_plist"
    fi
else
    echo -e "${YELLOW}No managed service found; removing files only.${NC}"
fi

# Remove the install directory (guarded: must contain the agent binary).
if [ -n "$target_dir" ] && [ -f "${target_dir}/agent" ]; then
    echo "Removing install directory: ${target_dir}"
    rm -rf "$target_dir"
else
    rm -f "${target_dir}/agent" 2>/dev/null || true
fi

echo -e "${GREEN}Komari Agent uninstalled.${NC}"
echo -e "${YELLOW}Tip: remove the node from the Komari panel to clear its offline entry.${NC}"
EOF

    chmod +x "$uninstall_path"
    log_success "Uninstall script ready: ${GREEN}$uninstall_path${NC}"
}
...
if [ -n "$github_proxy" ]; then
    ...
    download_url="${github_proxy}/https://github.com/xshhhlol/komari-agent/releases/${download_path}/${file_name}"
else
    # Direct access to GitHub releases
    download_url="https://github.com/xshhhlol/komari-agent/releases/${download_path}/${file_name}"
fi
...
log_config "Uninstall: ${GREEN}${target_dir}/uninstall.sh${NC}"
echo -e "${WHITE}===========================================${NC}"
