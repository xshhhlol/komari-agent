...
if ($InstallVersion -ne "") {
    ...
    $ApiUrl = "https://api.github.com/repos/xshhhlol/komari-agent/releases/latest"
    try {
...
$BinaryName = "komari-agent-windows-$arch.exe"
$DownloadUrl = if ($GitHubProxy) { "$GitHubProxy/https://github.com/xshhhlol/komari-agent/releases/download/$versionToInstall/$BinaryName" } else { "https://github.com/xshhhlol/komari-agent/releases/download/$versionToInstall/$BinaryName" }
...
# Leave a self-contained uninstall script next to the binary
function New-UninstallScript {
    $uninstallPath = Join-Path $InstallDir "uninstall.ps1"
    Log-Step "Creating uninstall script: $uninstallPath"

    # Header: bake the install-time service name / dir into the script.
    $header = @"
# Komari Agent uninstall script (auto-generated by install.ps1).
# Stops and removes the agent service via nssm, then deletes the install directory.
`$ServiceName = "$ServiceName"
`$InstallDir = "$InstallDir"
"@

    # Body: single-quoted here-string so these vars resolve at uninstall time, not now.
    $body = @'
...
Write-Host "Uninstalling Komari Agent (service: $ServiceName)..." -ForegroundColor Yellow

# Prefer the local nssm.exe shipped with the install
...
try {
    ...
    # Fallback to sc.exe for older installs
    $svc = Get-Service -Name $ServiceName -ErrorAction SilentlyContinue
    if ($svc) {
        Stop-Service $ServiceName -Force -ErrorAction SilentlyContinue
        sc.exe delete $ServiceName | Out-Null
...
# Remove the install directory (guarded: must contain the agent binary).
# Deletion is detached so it can remove the folder holding this running script + nssm.exe.
$agent = Join-Path $InstallDir "komari-agent.exe"
if ((Test-Path $agent) -and (Test-Path $InstallDir)) {
    $cmd = "timeout /t 2 >nul & rmdir /s /q `"$InstallDir`""
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c", $cmd -WindowStyle Hidden
    Write-Host "Install directory scheduled for removal: $InstallDir" -ForegroundColor Green
}

Write-Host "Komari Agent uninstalled." -ForegroundColor Green
Write-Host "Tip: remove the node from the Komari panel to clear its offline entry." -ForegroundColor Yellow
'@

    Set-Content -Path $uninstallPath -Value ($header + "`n" + $body) -Encoding UTF8
    Log-Success "Uninstall script ready: $uninstallPath"
}
New-UninstallScript
...
Log-Config "Uninstall: $(Join-Path $InstallDir 'uninstall.ps1')"